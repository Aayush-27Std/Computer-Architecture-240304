...
The generated waveforms show that the encoder produces the correct priority-coded outputs when multiple inputs are active; the valid signal (`V`) indicates when any input is asserted. The decoder shows a single active output corresponding to the input code when `EN` is high and all outputs low when `EN` is low.

## Discussion and Conclusion

This lab implemented a 4-to-2 priority encoder and a 2-to-4 decoder in VHDL and verified them using GHDL and GTKWave. The encoder was implemented using a priority-checking behavioral architecture to ensure the highest-priority input is encoded when multiple inputs are active. The decoder was implemented using a case statement with an active-high enable signal. Testbenches exercised normal, boundary, and priority conditions. The waveforms matched the expected truth tables and behavior, confirming the correctness of both designs. This lab reinforced behavioral modeling in VHDL and the workflow for simulation and waveform inspection, preparing for more complex combinational and sequential designs in later labs.
